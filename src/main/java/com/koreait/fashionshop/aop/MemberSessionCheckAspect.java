...
//앞으로 로그인이 필요한 서비스를 처리하기 위한 코드는, 컨트롤러에 두지않고
...
public class MemberSessionCheckAspect {
	...
	public Object sessionCheck(ProceedingJoinPoint joinPoint) throws Throwable{
		...
		Object[] args = joinPoint.getArgs(); //원래 호출하려했던 메서드의 매게변수
		
		//현재의 요청이 세션을 가지고 있는 지를 판단하여, 적절한 제어..
		...
		if (session.getAttribute("member") == null) {
			...
			//원래 요청의 흐름을 그대로 진행..
			//원래 호출하려했던 메서드를 대신 호출해준다.
			result = joinPoint.proceed(); //여기서 예외가 발생하므로 예외처리하지말고 그냥 throws!
		}
